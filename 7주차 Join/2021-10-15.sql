...
ALTER TABLE 상영관 ADD CONSTRAINT CHECK (상영번호 BETWEEN  1 AND 10);
ALTER TABLE 예약 MODIFY COLUMN 날짜 DATE;  
ALTER TABLE 예약 CHANGE COLUMN 날짜 날짜 DATE;
...
CREATE VIEW 극장_고객
AS SELECT 극장.극장이름, 고객.이름 
FROM 극장, 고객, 예약
WHERE 극장.극장번호 = 예약.극장번호 AND 고객.고객번호 = 예약.고객번호
WITH CHECK OPTION;

CREATE OR REPLACE VIEW 극장_고객 -- 이중 조인
AS SELECT 극장.극장이름, 고객.이름 
FROM 예약 JOIN 고객 ON 고객.고객번호 = 예약.고객번호
...
CREATE OR REPLACE VIEW 대한_고객수
AS SELECT 날짜, COUNT(*) AS  '예약한 고객의 수'
FROM 예약
WHERE 극장번호 IN (SELECT 극장번호
									FROM 극장
									WHERE 극장이름 = '대한')
