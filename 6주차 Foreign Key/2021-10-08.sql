...
DROP TABLE IF EXISTS 의사, 환자;

SELECT 환자번호, 환자이름
FROM 환자
WHERE 담당의사 = 'D001' AND 나이 >= 30;

SELECT 소속, COUNT(*) AS '의사 수', AVG(근무연수) AS '평균 근무연수' FROM 의사
GROUP BY 소속
ORDER BY 소속 asc;
...
SELECT 의사이름, 소속, 근무연수
FROM 의사 JOIN 환자 ON 환자.담당의사 = 의사.의사번호
WHERE 환자이름 = '김용욱';

-- 의사번호 그리고 환자이름으로 서브쿼리를 사용할 수 있어야 할 것,
SELECT 의사이름, 소속, 근무연수 
FROM 의사
WHERE 의사번호 = (SELECT 담당의사 FROM 환자 WHERE 환자이름 = '김용욱');

SELECT 의사이름, 소속, 근무연수 
FROM 의사, 환자
WHERE 환자이름 IN ('김용욱') AND 의사번호 = 담당의사;

SELECT 의사이름, 소속, 근무연수 
FROM 의사
WHERE EXISTS (SELECT * FROM 환자 WHERE 환자이름 = '김용욱' AND 의사번호 = 담당의사);
